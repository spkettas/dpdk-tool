...
static void print_usage(const char* prgname) {
  RTE_LOG(INFO, APP,
          ...
          "[--config (port,lcore_rx,lcore_tx)"
          "[,(port,lcore_rx,lcore_tx)]]\n"
...
static int parse_args(int argc, char** argv) {
  ...
  /* Parse command line */
  while ((opt = getopt_long(argc, argv, "p:", longopts, &longindex)) != EOF) {
    switch (opt) {
      case 0:
...
static int kni_free_kni(uint16_t port_id) {
  ...
  for (i = 0; i < p[port_id]->nb_kni; i++) {
    if (rte_kni_release(p[port_id]->kni[i])) printf("Fail to release kni\n");
...
int main(int argc, char** argv) {
  ...
  /* set port mask*/
  for (j = 0; j < nb_sys_ports; ++j) {
    ports_mask = ports_mask | (1 << j);
  }
  printf("get ports_mask=%u\n", ports_mask);

  /* Parse application arguments (after the EAL ones) */
  ret = parse_args(argc, argv);
  if (ret < 0) rte_exit(EXIT_FAILURE, "Could not parse input parameters\n");
